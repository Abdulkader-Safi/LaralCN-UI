...
<body class="bg-background text-foreground antialiased">
    @php
        $navLinks = [
            ['route' => 'docs.getting-started', 'label' => 'Getting Started', 'match' => 'docs.getting-started'],
            ['route' => 'docs.index', 'label' => 'Components', 'match' => 'docs.index'],
            ['route' => 'blocks.index', 'label' => 'Blocks', 'match' => 'blocks.*'],
            ['route' => 'docs.theming', 'label' => 'Theming', 'match' => 'docs.theming'],
            ['route' => 'docs.plain-blade', 'label' => 'Plain Blade', 'match' => 'docs.plain-blade'],
        ];
    @endphp
            ...
            <x-ui.sidebar.header>
                <x-ui.sidebar.menu>
                    <x-ui.sidebar.menu-item>
                        <x-ui.sidebar.menu-button size="lg" href="{{ route('home') }}">
                            <div
                            ...
                            <div class="grid flex-1 text-left text-sm leading-tight">
                                <span class="truncate font-semibold">LaralCN-UI</span>
                        ...
                        @foreach ($navLinks as $link)
                            <x-ui.sidebar.menu-item>
                                <x-ui.sidebar.menu-button :href="route($link['route'])"
                                    :tooltip="$link['label']"
                                    :is-active="request()->routeIs($link['match'])">
                    ...
                    <x-ui.sidebar.group>
                        ...
                        <x-ui.sidebar.menu>
                            @foreach ($items as $item)
                                <x-ui.sidebar.menu-item>
                                    <x-ui.sidebar.menu-button
                                        :href="route('docs.show', $item['name'])"
                                        :tooltip="$item['name']"
                                        :is-active="isset($entry) && $entry['name'] === $item['name']">
                                        <span>{{ $item['name'] }}</span>
                                    </x-ui.sidebar.menu-button>
