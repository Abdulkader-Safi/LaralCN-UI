{{--
    ...
    A featured item is ['label' =>, 'description' =>, 'href' =>, 'image' => url];
    without an image it draws a grey placeholder.
    ...
    <div class="group/mega relative mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8"
        ...
        <div class="lg:hidden">
            <x-ui.sheet side="right">
                ...
                <x-slot:footer>
                    @foreach ($actions as $action)
                        <x-ui.button :variant="$action['variant'] === 'ghost' ? 'outline' : ($action['variant'] ?? 'default')"
